...
def _to_int(s):
    ...
    try:
        ss = str(s).strip()
        if ss == "" or ss.lower() in {"none", "nan", "null", "-"}:
            return None
        # some PDFs yield '1 0' or similar; take last token
        ss = ss.split()[-1]
        return int(float(ss))
    except Exception:
        return None
        ...
        for ln in lines:
            ...
            if m and SKU_RE.match(m.group(2)):
                ...
                current = {
                    ...
                    "sku": sku,
                    "description": desc,
                    "ordered": _to_int(ordered),
                    "shipped": _to_int(shipped),
                    "balance": _to_int(balance),
                    "price": _to_float_money(price),
                    "total": _to_float_money(total),
                }
